...
const getAllVideos = asyncHandler(async (req, res) => {
    ...
    // Only fetch published videos unless filtering by userId
    if (!userId) {
        pipeline.push({
            $match: {
                isPublished: true
...
const togglePublishStatus = asyncHandler(async (req, res) => {
    ...
    return res
        .status(200)
        .json(new ApiResponse(200, video, `Video publication status changed to ${video.isPublished ? "published" : "unpublished"}`))
})
